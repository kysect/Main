...
Цель тестового задания - стать первым шагом сектанта в мир C#. Его выполнение подразумевает краткое знакомство с различными инструментами языка. После выполнения задания, его будут ревьювить менторы секты, которые смогут дать фидбек и посоветовать что-то дополнительно изучить, чтобы закрыть свои пробелы.

Основная задача - реализовать консольную программу для управления задачами. Выполненное задание нужно загрузить на в свой репозиторий на гитхабе, отправить пул-реквест. Если гитхаб является проблемным местом, можно начать с [этого гайда](articles/working-with-github/README.md).
...
- Стоит сделать акцент на Объектно-ориентированном решение, попробовать выделить классы. Например, хорошим решением является отделить логику выполнения от логики чтения/записи на консоль
