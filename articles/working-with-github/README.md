...
Git - система контроля версий. С его помощью мы храним все версии редактируемых файлов и можем вернуться к любой сохраненной версии в любой момент времени. Кроме того git поддерживает возможность использования одной системы контроля на проекте множеством разработчиков. 
...
Сам репозиторий представляет собой каталог файловой системы, в котором хранится ваш проект и вещи, необходимые ему для корректной работы. Репозитории бывают локальными (лежит у нас на компе) и удалёнными (например лежит на GIthub). Эти репозитории связаны между собой, но изменения в одном из них не меняют другой автоматически. 
